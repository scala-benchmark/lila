...
object LoggerConfigurator:
  ...
  def configure(properties: Map[String, String], configUrl: URL, configData: String = ""): Either[Unit, String] =
    if configData.nonEmpty then
      ...
      //CWE 611
      //SINK
      val xmlDoc = scala.xml.XML.withSAXParser(saxParser).loadString(configData)
      Right(xmlDoc.toString())
    else
